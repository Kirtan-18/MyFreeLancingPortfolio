...
export const Navbar: React.FC = () => {
  ...
  const { isBackendConnected } = usePortfolio();
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          {/* Right Controls: Audio Toggle, DB Status, Hidden Holographic Orb */}
          <div className="hidden lg:flex items-center gap-3">
            {/* DB Handshake Badge */}
            <div
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-mono glass-panel border-white/10"
              title={isBackendConnected ? 'Connected to MongoDB Atlas Backend' : 'Offline / Local State Mode Active'}
            >
              <Database className={`w-3 h-3 ${isBackendConnected ? 'text-emerald-400' : 'text-amber-400'}`} />
              <span className={isBackendConnected ? 'text-emerald-400' : 'text-amber-400'}>
                {isBackendConnected ? 'LIVE DB' : 'LOCAL SYNC'}
              </span>
            </div>
            ...
            {/* Hidden Admin Access: Holographic Orb ("⚡ CLICK TO BOOM") */}
            <HolographicOrb onClick={handleOrbClick} />
          </div>
